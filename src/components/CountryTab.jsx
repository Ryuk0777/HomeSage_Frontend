...
const CountryTab = ({src, title}) => {
  ...
  return (
    <>
      <button onClick={()=> {dispatch(navigatToPage(title)); dispatch(toggleSidebarState()); dispatch(setData({}))}} className='w-full h-[12vh] bg-slate-200 border border-white flex items-center gap-x-10 px-2 hover:bg-slate-300'>
